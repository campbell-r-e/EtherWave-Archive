...
| 13 | YAML-Driven Configurable Contest Validator | **Planned (High Priority)** | Migrate contest configs JSON → YAML; generic `ConfigurableContestValidator`; new contests via YAML only, no Java required |
...
|---------|----------|-------|
| YAML-driven configurable contest validator (Epic 13) | **High** | Migrate all 7 contest configs from JSON to YAML. Build `ConfigurableContestValidator` — a generic Spring `@Component` that reads `rulesConfig` YAML at runtime and validates exchange fields declaratively (required fields, types, allowed values, shared value lists). New contests added by dropping a `.yaml` file in `contest-configs/` with no Java code needed. `validator_class` field in YAML can still point to a custom Java class for contests requiring cross-QSO logic (e.g. SOTA minimum 4 QSOs, POTA park reference lookup). `DataInitializationService` updated to scan `*.yaml` instead of `*.json`. Dependencies: snakeyaml (already a Spring Boot transitive dep, pin version explicitly). See full spec in `AGILE_PRODUCT_SPECIFICATION.md` Epic 13. |
