...
| 13 | YAML-Driven Configurable Contest Validator | **Planned (High Priority)** | Backend: JSON → YAML migration, `ConfigurableContestValidator`, `exchangeFields` in contest API. Frontend: dynamic exchange field rendering, contest list from backend API (no hardcoding) |
...
|---------|----------|-------|
| YAML-driven configurable contest validator (Epic 13) | **High** | **Backend:** Migrate all 7 contest configs JSON → YAML; `ConfigurableContestValidator` Spring `@Component` validates exchange fields declaratively from YAML; `DataInitializationService` scans `*.yaml`; `ContestController` response extended with `exchangeFields` array (resolved from YAML `required_fields` + `optional_fields`). **Frontend:** `ExchangeField` interface + `Contest.exchangeFields` in `station.model.ts`; `QsoEntryComponent.onContestChange()` replaced with dynamic field init; `qso-entry.component.html` hardcoded `@if` blocks replaced with `@for` loop; `log-selector` contest list driven from backend API so YAML-only contests appear without code changes. Custom Java validator still supported via `validator_class` in YAML (for SOTA/POTA cross-QSO logic). See full spec in `AGILE_PRODUCT_SPECIFICATION.md` Epic 13. |
